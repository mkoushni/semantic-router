...
func TestProcessResponse_BatchSelectionLogic(t *testing.T) {
	...
	// Advance to turn 5 (batch boundary)
	for i := 1; i <= 5; i++ {
		err := extractor.ProcessResponse(ctx, "session1", "user1", history)
		require.NoError(t, err)
	}

	// Verify the LLM received only the last batchSize+5=10 messages
	...
	// Messages 10-19 SHOULD be included
...
func TestProcessResponse_BatchSelectionSmallHistory(t *testing.T) {
	...
	// Create only 5 messages (less than batchSize+5=15)
	history := []Message{
	...
	// Advance to turn 10 (batch boundary)
	for i := 1; i <= 10; i++ {
		err := extractor.ProcessResponse(ctx, "session1", "user1", history)
		require.NoError(t, err)
	}

	// Verify ALL messages were included (history < batchSize+5)
	require.NotNil(t, capturedBody, "LLM should have been called")
	bodyStr := string(capturedBody)
